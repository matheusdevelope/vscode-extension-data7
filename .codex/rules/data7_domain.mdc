...
- Linter diagnostics use short, stable string codes declared in `src/diagnostics/diagnostic-codes.ts` as a frozen `DiagnosticCodes` object. The canonical set currently includes: `missing-import`, `unused-import`, `duplicate-import`, `unknown-member`, `module-not-found`, `module-not-declared`, `private-member-access`, `event-signature-mismatch`, `unsupported-member`, `not-enumerable`, `unknown-suppression-code`, `invalid-interpolation`, `ternary-context-unsupported`. New codes follow `kebab-case` and must be added to `DiagnosticCodes` AND documented in `project_context.md` § 4.4 AND illustrated by at least one canonical example under `docs/example/diagnostics/<code>/` before any use in code. Each code should ship with a typed payload (see `MissingImportPayload`, `UnknownMemberPayload`, etc.) attached to `Diagnostic.data` so code actions remain message-agnostic.
