...
- O sugar `array-list` agora expande `map` e `filter` com arrow block terminando em `Return`, alem de `Return lista.map(...)`/`Return lista.filter(...)` em funcoes que retornam `TTList_*`, evitando que `=>` seja serializado no `.7Proj` final.
