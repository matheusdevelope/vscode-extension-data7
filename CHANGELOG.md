...
- Chamadas encadeadas em `TTList` agora podem quebrar linha apos o ponto (`lista.Filter(...).Map(...).Reduce(...)` em formato multiline), preservando o receiver correto entre tipos genericos materializados como `TTList_Produto`. `Return` dentro de `Function` lambda deixa de gerar `return-unrecommended`, pois e a forma obrigatoria de retorno nesse contexto.
