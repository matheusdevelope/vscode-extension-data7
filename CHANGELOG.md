...
### Corrigido
- **Build — prune + `logger-print`:** com `prune.enabled`, a descoberta de generics rodava no fonte pré-transpile e não via `mod_logger` (`Print` só vira `mod_logger.Printe` depois). `TTList<LogTransport>` não era pedido, mas o prune pós-sugar mantinha o módulo reescrito → `Identificador não declarado "TTList_LogTransport"`. Namespaces de runtime dos sugars (`mod_logger`) passam a entrar na descoberta.
- **Build — `TTList_*` aninhado em outro genérico:** `Dim result[] As TypeRow` / `As TTList<TypeRow>` dentro de `TTMatrix<TypeRow>` deixava de materializar `TTList_TGridRow` em `mod_tlist` (o pedido aberto era ignorado e o template externo só reescrevia o nome plano). O Builder fecha esses usos internos quando a classe externa é instanciada (`Inherits TTMatrix<TGridCol, TGridRow, Variant>`), gerando `TTList_TGridRow` / `TTList_TGridCol`.
- **Linter — falso `duplicate-declaration` em globais de `Principal.bas`:** `Dim _run` / `_rows` / `_cols` (e outros globais do entrypoint) não conflitam mais consigo mesmos quando a URI do índice e a do editor diferem em casing ou encoding (comum no Windows). A coleta de globais do Principal passa a usar `isSameFileUri` em vez de igualdade de string.
- **MCP / System Library — `Forms.Grid` alinhado à API nativa:** índices de `Cells` / `Col` / `Row` / `ColWidth` / `HideColumn` são **0-based** (não 1-based). `ColCount` conta só colunas não ocultas (`AllColCount` inclui ocultas; `VisibleColCount` é a viewport). `PermiteMarcarExclusao` documenta a coluna oculta extra e a ordem estrutural (flag por último; não usar `UnHideColumnsAll` com a flag ligada). `RealColIndex` não se mistura com `Cells`. Delegates: `TCanEditCellEvent` é `(Sender, ARow, ACol, ByRef CanEdit)`; `TDblClickCellEvent` é `(pSender, pRow, pCol)` (linha, depois coluna); `TCellValidateEvent` tem `ByRef AValue`. `scripts/generate-system-library-docs.js` passa a resolver `DocsGenerator` em `packages/data7-core/dist`.
- **Importação/publicação de módulos — `Principal.bas` e Namespace:** na publicação (local e online), `.bas` sem `Namespace` declarado são descartados (o `Principal.bas` de desenvolvimento do módulo isolado tipicamente não é publicado). Na sincronização/importação para `data7_modules/`, arquivos sem Namespace também são removidos e qualquer `Principal.bas` restante é 100% comentado para não atuar como entrypoint no projeto consumidor.
- **Build — generics + prune:** com `prune.enabled`, a descoberta de instanciações genéricas (`TTList<MyGridRow>`, etc.) passa a considerar só namespaces do closure do Principal **do projeto** (`src/Principal.bas`). `Principal.bas` de dependências em `data7_modules` não vira seed/entry e não força monomorphs órfãos em `mod_tlist` (ex.: `TTList_MyGridRow` / `TFindDel_MyGridRow`) depois que o prune remove os tipos argumentados.
- **Linter — falso `duplicate-declaration` em overloads com retorno distinto:** `First() As T` e `First(n As Integer) As TTList<T>` (e o par `Last`) são overloads válidos no Data7. A regra deixava de exigir retorno idêntico e passa a conflitar só por assinatura de parâmetros igual ou por colisão com membro não-callable.
- **Problems vazio após “Analisar projeto”:** o overlay de `unused-code` relia em `DiagnosticCollection.get`, que no Windows é sensível a casing da Uri e podia apagar os erros já publicados; o resumo ainda contava o lote em memória. A publicação agora (1) recria diagnósticos com `vscode.Diagnostic`/`Range` do host, (2) guarda a base em mapa próprio para o overlay e (3) conta o toast a partir da collection/`publishedBaseByUri` após o merge.
 `data7://idioms` (preamble) e `docs/linguagem-basic/` (§01, §08, §12, §13) passam a orientar agentes: nome de `Namespace` só `[A-Za-z_][A-Za-z0-9_]*` (sem pontos); nunca `Imports` do próprio namespace; imports circulares proibidos (`circular-import`) — reestruturar ou usar `nome_do_namespace.Membro` qualificado. Prompt `data7_module_skeleton` reforça o charset do namespace.
...
### Alterado
- **Build / Executar — menos análise duplicada:** `validateTranspiled` deixa de relintar o código materializado com o walker strict (isso dominava o cold/hot build). O gate passa a ser só parse nativo (`assertTranspiledNativeSyntax`). O F5 (`lintWorkspaceForRun`) reutiliza AST/cache semântico em vez de `prepareFreshWorkspaceAnalysis`, não dispara workers nem o overlay `unused-code`, e o debounce de `unused-code` no editor sobe de 600 ms para 1500 ms. `findClassSymbol` ganha `MemberCache` por nome+contexto; o passe de generics do linter reusa a AST do `LanguageProcessor`.
- **Lambdas — tipos qualificados na materialização:** parâmetros anotados como `ns.Tipo` na lambda fonte são preservados no método gerado `__data7_lambda_*` (em vez de colapsar para o nome simples vindo da assinatura do delegate monomorfizado). Evita `Identificador não declarado` / `tipo não declarado` quando o arquivo não tem `Imports` do namespace.
- **Generics — metaprogramação `TypeSystem.IsKind`:** além de `TypeSystem.InheritsFrom(T, "Base")`, templates genéricos aceitam `TypeSystem.IsKind(T, "Delegate"|"Class"|"Structure"|"Primitive"|"Enum"|"Unknown")` e atalhos `IsDelegate` / `IsClass` / `IsStructure` / `IsPrimitive` / `IsEnum` (com `NOT` opcional). Permite ramificar a materialização quando o argumento concreto é um Delegate, primitivo, etc.
- **Sugar `Enun` — guard de `Initialize`:** deixa de emitir o campo `Private Shared _Initialized` e passa a usar `If TEnum._IsCached("<Enum>", "<primeiroItem>") Then Exit Sub` (alinhado a `mod_tenum.TEnum._IsCached` / `_AddEnumItem`).
- **Sugar `array-list` — atribuição de literal `[]`/`[...]` a TTList:** `me.lista = []` e `me.lista = [x]` (e o mesmo para `Dim` já tipado como lista) materializam `me.lista = New TTList_T()` + `Push` por elemento, em vez de deixar o literal de array no bytecode.
- **`For Each` / `detectEnumerable` — TTList usa `GetItem`:** a precedência de indexers passa a ser `Items` > `GetItem` > `Take` > `Item` > `Strings` > `Objects`. Assim `TTList`/`TTList_*` (que herdam `Item As TTObject` de `TTComposerList`) materializam `For Each` com `.GetItem(i)` em vez de `.Item(i)`. O hint `As ns.Tipo` também casa com o tipo simples do indexer.
- **Sugar `array-list` — `Imports mod_tlist`:** materializar `p[] As T` / `Dim x[] As T` marca o sugar e injeta `Imports mod_tlist` via `requiredImports`. O builder também resolve `TTList_*` para o namespace do template `TTList` quando o monomorfo ainda não está indexado.
- **Linter — `lista(i)` em TTList/array-sugar:** `pColumns(i)` resolve como o elemento `T` (não `TTList<T>`). Em `TTList` real (`Inherits TTComposerList`), o `Item As TTObject` herdado não prevalece sobre o argumento genérico — evita falso `type-mismatch` `TTObject` → `TGridColumnDef`.
- **Sugar `array-list` — índice com parênteses em TTList:** `lista(i)` (forma VB parseada como chamada) sobre binding conhecido de `TTList`/`[]` passa a materializar `lista.GetItem(i)`; atribuição `lista(i) = v` vira `lista.SetItem(i, v)`. Parâmetros `p[] As T` entram no escopo de listas do método para que `p(i)` seja reescrito (ex.: `pColumns(i)` → `pColumns.GetItem(i)`). Campos sugar no receptor também: `pConfig.Columns(i)` → `pConfig.Columns.GetItem(i)` e `me.Columns(n) = v` → `me.Columns.SetItem(n, v)`. Métodos de um argumento e arrays nativos (`_columns(64)`) permanecem intactos.
- **`npm run watch`:** após um compile limpo de `@data7/core` ou `vscode-extension-data7`, o watcher reempacota automaticamente `out/extension.js`, `out/lint-worker.js` e (quando o core muda) o MCP `server.bundled.js`. O Extension Host carrega esses bundles — só `tsc` deixava o F5 com lógica stale.
- **MCP `data7_TEnum_pattern`:** default passa a gerar sugar `Enun X / End Enun` + guia da API materializada (factories, `Load`, `GetOptions`, `.AsString`/`.IsValue`) em vez da classe `Inherits TEnum`. A forma expandida permanece via `form: "expanded"` só para customização. Idioms, convenções §1 e docs MCP alinhados.
...
### Corrigido
- **Problems vazio após lint de workspace:** o resumo podia contar erros enquanto a collection não retinha os mesmos diagnósticos — `get(uri)` com casing diferente, offload stale do `AnalysisProgram` no batch e cache semântico entre passes. O lint de workspace agora força check síncrono, publica os diagnósticos calculados (não o `get` da collection), limpa o `SemanticLintCache` no prepare e abre o painel Problems quando há erros.
- **Linter — membros de `Enun` / TEnum:** o indexador materializa `Enun` como classe com base `mod_tenum.TEnum` (evita o homônimo legado `mod_enum.TEnum` “roubar” a herança). `AsString`, `IsValue`, `Load`, `GetOptions` e factories deixam de gerar falso `unknown-member` mesmo sem `Imports mod_tenum` no arquivo do `Enun`. `Enum` nativo permanece sem herança TEnum.
- **Linter — `duplicate-declaration` em membros de classe:** `Shared` e instância passam a compartilhar a mesma tabela de nomes (ex.: campo `Mask As String` + `Shared Function Mask(...)` passa a emitir erro). Overloads só são aceitos com o mesmo tipo de retorno e parâmetros distintos; assinatura idêntica ou retorno divergente emite _Error_ (antes assinatura idêntica era só _Warning_, e Shared/instância ficavam isolados).
- **Sugar `Enun` / padrão TEnum:** a materialização deixa de emitir construtor local (`Private Sub New`) e `CStr`/`CType` — reutiliza `TEnum.New(Integer, String)`, gera três overloads de `Load` (`X` / `Integer` / `String`) com cast `X(TEnum._GetCache(...))`, e encapsula valores numéricos de entrada em String (`23` → `"23"`). Prompt MCP `data7_TEnum_pattern`, exemplo canônico, convenções e linter (`missing-mybase-new`/`missing-mybase-free` em subclasses de `TEnum`) alinhados.
- **Sugar `array-list` — parâmetros `pList[] As T`:** o parser passa a aceitar o marcador `[]` em parâmetros de `Function`/`Sub`/`Delegate`/`Declare`/lambda (antes falhava com `expected-token` em `[`). Materializa para `TTList_T` no transpile e o linter resolve o parâmetro no escopo do método (sem falso `unknown-symbol` / `missing-return-type`). Exemplo canônico: `docs/example/sugar/array-list/05-parameter-array-sugar.bas`.
