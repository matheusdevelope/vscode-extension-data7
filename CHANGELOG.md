...
### Corrigido
- **Cache de overloads em chamadas não qualificadas:** `findUnqualifiedCallable` e o cache de tipo de retorno de invocações agora distinguem múltiplas chamadas na mesma linha (posição no arquivo) e overloads com a mesma aridade pelos tipos dos argumentos. Corrige falsos `type-mismatch` em padrões como `EnumToInt(TFileMode.Open()), EnumToInt(TFileAccess.Read())` no mesmo statement.
