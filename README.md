...
- `duplicate-declaration` trata `Shared` e instancia na mesma tabela de nomes (campo + factory `Shared Function` com o mesmo nome colidem) e aceita overload de metodo com parametros distintos (o tipo de retorno pode divergir, como em `First()` / `First(n)` do `TTList`). Globais de `Principal.bas` nao sao reportados como conflito contra o proprio arquivo.
...
- `Forms.Grid` usa índices **0-based** (`Cells`, `Col`, `Row`, `ColWidth`, `HideColumn`). `ColCount` são as colunas não ocultas (`AllColCount` inclui ocultas). `OnCanEditCell` recebe `(Sender, ARow, ACol, ByRef CanEdit)`; `OnDblClickCell` recebe `(pSender, pRow, pCol)`; `OnCellValidate` tem `ByRef AValue`.
...
- Antes de iniciar o Executor no F5, o projeto é reanalisado pelo parser/linter (reusando o cache quente do editor) e a execução é cancelada se houver erro. Chamadas qualificadas em namespaces/tipos, como `console.clear()`, também são validadas como `unknown-member` quando o membro não existe. O build valida só a sintaxe nativa da saída transpilada, não relinta o walker no código materializado.
...
- **Language Server (experimental):** com `features.diagnostics.useLanguageServer: true` nos settings da extensão, o lint passa a rodar no processo `@data7/lsp` (fora do extension host) e é o único publicador do painel Problems. O default continua `false` até a migração estabilizar.
...
- O monomorfizador materializa retornos genericos fluentes de qualquer classe concreta, evitando metodos ausentes em cadeias como `Classe<T>.Metodo<TOut>() As OutraClasse<TOut>` e descartando usos ainda abertos como `Classe<T>` em comentarios ou templates. Instanciações aninhadas entre arquivos (`TTMatrix<TGridRow>` que devolve `TTList<TypeRow>`) passam a pedir `TTList_TGridRow` no módulo dono do template. Com prune, a descoberta inclui `mod_logger` (sugar `logger-print`) para materializar `TTList_LogTransport`.
...
- Literais `[a, b]` e `Dim x[] As T = []` usam tipagem contextual do destino (`TTList<T>` / parâmetro); parâmetros `pList[] As T` materializam para `TTList_T`; índice `lista(i)`/`lista[i]` (e `obj.Lista(i)` em campo sugar) vira `GetItem`/`SetItem`; atribuição `lista = []`/`lista = [a]` a TTList vira `New`+`Push`; `For Each` itera `TTList<T>` via `Length`+`GetItem`/`Take` (e `Count` é alias de `Length`).
...
- **Sincronização**: A extensão baixa e sincroniza automaticamente dependências ausentes para a pasta `data7_modules/` e mantém `data7_modules/core_modules/` alinhado com a versão instalada da extensão. Na importação, `.bas` sem `Namespace` são descartados e `Principal.bas` (entrypoint de desenvolvimento do módulo isolado) é comentado por completo. Na publicação, só entram `.bas` com Namespace. O construtor injeta automaticamente a flag `@Module-Imported` em arquivos de dependência externa para evitar conflitos na decomposição.
