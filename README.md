...
- **Linter** com diagnósticos canÃ´nicos (`missing-import`, `unused-import`, `unknown-member`, `module-not-found`, `module-not-declared`, `duplicate-import`, `private-member-access`, `event-signature-mismatch`).
...
- Lambdas usam sintaxe VB-like (`Function(...) expr`, bloco `Function ... End Function` ou `Sub ... End Sub`) e sao validadas contra delegates pelo linter, incluindo o corpo da lambda e os tipos dos argumentos nas chamadas internas. Elas podem ser passadas como argumentos em chamadas multiline, chamadas encadeadas com ponto no fim da linha e metodos genericos como `Reduce<Double>(...)`. Em `Function` lambda, use `Return` para devolver valores em blocos; retorno por atribuicao ao nome da funcao nao se aplica porque a lambda nao tem nome. O sugar `array-list` aproveita esse suporte para expandir `map`, `filter`, `find`, `findIndex`, `some`, `every`, `reduce` e `forEach` sobre `TTList`, inclusive em `Return lista.map(...)` de funcoes que retornam `TTList_*`; `Map<TOut>` infere o tipo de saida pelo retorno da lambda e `Reduce<TAcc>` materializa handlers com a assinatura completa do delegate.
