...
- **Language Server (experimental):** com `features.diagnostics.useLanguageServer: true` nos settings da extensão, o lint passa a rodar no processo `@data7/lsp` (fora do extension host). O default continua `false` até a migração estabilizar.
