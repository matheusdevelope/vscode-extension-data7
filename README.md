...
- `duplicate-declaration` trata `Shared` e instancia na mesma tabela de nomes (campo + factory `Shared Function` com o mesmo nome colidem) e aceita overload de metodo com parametros distintos (o tipo de retorno pode divergir, como em `First()` / `First(n)` do `TTList`).
