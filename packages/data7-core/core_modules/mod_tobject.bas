...
Namespace mod_tobject
    ...
    Class TTObject
        ...
        Overridable Sub Assign(pValue As TObject)
            Throw New Exception("You must to implement this method, do NOT use the Overrides, just declare: Sub Assign(pValue As " & CStr(me.Classname()) & ")")
        End Sub

        Overridable Function Clone() As TObject
            Throw New Exception("You must to implement this method: Function Clone() As " & CStr(me.Classname()) & "")
        End Function
        ...
        Private Sub TriggerEvent(pList As TTObjectList, pItem As TTObject, pIndex As Integer, pMethod As OnActionListDel, pDelegate As OnActionListDel)
            If pMethod <> Null Then
                pMethod(pList, pItem, pIndex)
            End If

            If pDelegate <> Null Then
                pDelegate(pList, pItem, pIndex)
            End If
    ...
    Class TTObjectList
        ...
        Sub New(pName As String = "TTObjectList", pOwnsObjects As Boolean = True, pDisableEvents As Boolean = False)
            MyBase.New()
            me.Name = pName
            me._list = New StringList()
            me._map = CreateObject("Scripting.Dictionary")
            me._map.CompareMode = 1
            me.OwnsObjects = pOwnsObjects
            me.DisableEvents = pDisableEvents
        End Sub

        Private Sub RebuildMap(pStartIndex As Integer = 0)
            ...
            For i = pStartIndex To _length - 1
                Dim _item As TTObject = CType(me._list.Objects(i), TTObject)
                If _item <> Null Then
                    me._map.Item(Trim(_item.GetID())) = i
                End If
            Next
        End Sub
        ...
        Sub Push(pValues As TTComposerList)
            Dim i As Integer
            Dim _length As Integer = pValues.Length
        ...
        Sub Unshift(pIndex As Integer, pValues As TTComposerList)
            Dim i As Integer
            Dim _length As Integer = pValues.Length - 1
            For i = 0 To _length
        ...
        Function IndexOf(pID As String) As Integer
            pID = Trim(pID)
            If me._map.Exists(pID) Then
                IndexOf = CInt(me._map.Item(pID))
            Else
                IndexOf = -1
            End If
        End Function
        ...
        Function Clone() As TTObjectList
            Dim _Clone As New TTObjectList()
            Dim i As Integer
            Dim _length As Integer = me.Length()
            For i = 0 To _length - 1
        ...
        Sub Delete(pIndex As Integer)
            Dim _item As TTObject = me.Take(pIndex)
            Dim _id As String = Trim(_item.GetID())
            If me._map.Exists(_id) Then
                me._map.Remove(_id)
            End If
        ...
        Sub Move(pFromIndex As Integer, pToIndex As Integer)
            me._list.Move(pFromIndex, pToIndex)
            Dim minIndex As Integer = pFromIndex
            If pToIndex < pFromIndex Then
                minIndex = pToIndex
            End If

            me.RebuildMap(minIndex)
        End Sub

        Sub Move(pIDFrom As String, pIDTo As String)
            me.Move(me.IndexOf(pIDFrom), me.IndexOf(pIDTo))
        End Sub
        ...
        Sub Sort(pHandler As SortPropDel, pAsc As Boolean = True, pExtra As Variant)
            If pHandler <> Null Then
                Dim listTemp As New StringList()
                Dim i As Integer
                Dim _length As Integer = me.Length()
                For i = 0 To _length - 1
        ...
        Private Sub DispatchBeforePush(pList As TTObjectList, pItem As TTObject, pIndex As Integer)
            If pItem <> Null Then
                pItem.DispatchBeforePush(pList, pItem, pIndex)
            End If
            me.TriggerEvent(pList, pItem, pIndex, me.BeforePush, me.OnBeforePush)
        End Sub
        ...
        Private Sub DispatchAfterPush(pList As TTObjectList, pItem As TTObject, pIndex As Integer)
            If pItem <> Null Then
                pItem.DispatchAfterPush(pList, pItem, pIndex)
            End If
            me.TriggerEvent(pList, pItem, pIndex, me.AfterPush, me.OnAfterPush)
        End Sub
        ...
        Private Sub DispatchBeforeDelete(pList As TTObjectList, pItem As TTObject, pIndex As Integer)
            If pItem <> Null Then
                pItem.DispatchBeforeDelete(pList, pItem, pIndex)
            End If
            me.TriggerEvent(pList, pItem, pIndex, me.BeforeDelete, me.OnBeforeDelete)
        End Sub
        ...
        Private Sub DispatchAfterDelete(pList As TTObjectList, pItem As TTObject, pIndex As Integer)
            If pItem <> Null Then
                pItem.DispatchAfterDelete(pList, pItem, pIndex)
            End If
            me.TriggerEvent(pList, pItem, pIndex, me.AfterDelete, me.OnAfterDelete)
        End Sub
        ...
        Private Sub DispatchBeforeDispose(pList As TTObjectList, pItem As TTObject, pIndex As Integer)
            If pItem <> Null Then
                pItem.DispatchBeforeDispose(pList, pItem, pIndex)
            End If
            me.TriggerEvent(pList, pItem, pIndex, me.BeforeDispose, me.OnBeforeDispose)
        End Sub
        ...
        Private Sub DispatchAfterDispose(pList As TTObjectList, pItem As TTObject, pIndex As Integer)
            If pItem <> Null Then
                pItem.DispatchAfterDispose(pList, pItem, pIndex)
            End If
            me.TriggerEvent(pList, pItem, pIndex, me.AfterDispose, me.OnAfterDispose)
        End Sub
    ...
    Class TTComposerList
        ...
        Property DisableEvents As Boolean
            Get
                DisableEvents = me._base.DisableEvents
            End Get
            Set(pValue As Boolean)
                me._SetDisableEvents(pValue)
            End Set
        End Property

        Private Sub _SetDisableEvents(pValue As Boolean)
            me._base.DisableEvents = pValue
        End Sub
        ...
        Sub Move(pFromIndex As Integer, pToIndex As Integer)
            me._base.Move(pFromIndex, pToIndex)
        End Sub
