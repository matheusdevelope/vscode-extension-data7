...
4. Debounce por documento (400 ms) no `DiagnosticService` para `onDidChangeTextDocument`, com contador de geração que descarta execuções obsoletas; `onDidSave` dispara refresh imediato com propagação a dependentes do grafo de `Imports` (`Principal.bas` espalha para todos os `.bas`); `fixActiveFile` força também a reavaliação de arquivos que ainda têm Problems. O comando **Rodar Linter no Projeto** limpa Problems e invalida caches de check antes de republicar (sem reparse síncrono prévio de todo o projeto).
...
10. Caches de membros no `TypeResolver`/`WorkspaceSymbolIndexer` (`ownMembersForClassCache`, `inheritedMembersForClassCache`, `allMembersForTypeCache`) e filtro de `supportedNodeKinds` nas rules reduzem trabalho repetido no cold path do walker.
