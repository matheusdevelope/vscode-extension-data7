...
- `src/analysis/analysis-host.ts`: Fronteira de ambiente (`AnalysisHost`) — documentos abertos, pastas do workspace, `fs` (sync + async) e log. `installAnalysisHost` / `getAnalysisHost`; implementações `createVscodeAnalysisHost` (extension host) e `createNodeAnalysisHost` (CLI/MCP/testes/LSP futuro).
