...
O sugar `array-list` materializa `Dim itens[] As T` como `TTList_T` e expande operacoes funcionais sobre listas conhecidas (`map`, `filter`, `find`, `findIndex`, `some`, `every`, `reduce`, `forEach`) antes da serializacao. `map` e `filter` aceitam arrow de expressao e arrow block; no bloco, o ultimo statement deve ser `Return <expr>`, que vira o valor inserido ou a condicao do filtro. Quando uma `Function` retorna `TTList_*`, `Return lista.map(...)` e `Return lista.filter(...)` sao baixados para uma variavel temporaria, loop `For` e `Return` dessa temporaria, para que nenhuma sintaxe `=>` chegue ao `.7Proj`. Com `array-list` habilitado, o linter trata `TTList<T>` e `TTList_*` como tipos runtime do sugar e nao exige `Imports mod_tlist` no fonte, pois o transpiler injeta esse import na saida materializada.
