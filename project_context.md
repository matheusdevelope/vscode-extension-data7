...
O novo **ModuleOrchestrator** / **PackageManager** é responsável por instalar, atualizar e publicar módulos. Ao instalar um módulo (via interface ou comando), ele é baixado de repositórios locais (em `~/.data7/local_modules`) ou remotos (no GitHub) e copiado para a pasta `data7_modules/`. O construtor encarrega-se de adicionar a tag `@Module-Imported` aos arquivos em `data7_modules/` para diferenciar dependências externas do código do usuário residente em `src/` durante o processo de build e decompilação.
