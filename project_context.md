...
Toda `Class` deve declarar pelo menos um construtor `Sub New`; `Structure` continua isenta. O diagnostico `missing-mybase-new` cobre dois casos: ausencia total de construtor (`action: "create-constructor"`) e construtor sem chamada a `MyBase.New()` (`action: "insert-mybase-new"`). O Quick Fix correspondente deve criar `Sub New()` sem parametros com `MyBase.New()` ou inserir a chamada no construtor existente.
...
4. Debounce por documento (400 ms) no `DiagnosticService` para `onDidChangeTextDocument`, com contador de geração que descarta execuções obsoletas; `onDidSave` dispara refresh imediato com propagação a dependentes, enquanto a digitação mantém o caminho local leve na CPU.
