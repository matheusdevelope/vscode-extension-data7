...
Imports desconhecidos devem passar pela validacao de modulos e emitir `module-not-found`/`module-not-declared`. O warning `unused-import` so deve ser emitido para namespaces ou classes resolvidos pelo `WorkspaceSymbolIndexer` ou pela System Library, evitando tratar modulo ausente como import nao utilizado.
