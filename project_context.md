...
- `src/analysis/analysis-program.ts`: Orquestrador incremental (`AnalysisProgram`) que mantém um `FileSnapshot` **imutável** por arquivo (conteúdo, versão, hash, AST, bind e último check). `update` é o único ponto de entrada de edição; `closeDocument` libera memória mantendo o arquivo indexado, `deleteFile` o remove do índice e `renameFile` faz a troca atômica. Resultados de check são publicados atomicamente (guarda de versão + hash), nunca escritos no snapshot em uso, e o consumidor é avisado por `onCheckCompleted`.
