...
- Expandido para a classe específica do Enum que herda de `TEnum` (do namespace `mod_tenum`). A base herda de `TTObject`, preservando identidade, cópia e descarte para uso seguro em `TTList`. A classe gerada expõe Initialize lazy, Shared Function por valor, três overloads de `Load` (por instância, `Integer` e `String` via `X(TEnum._GetCache(...))`) e `GetOptions` — sem construtor local (usa o da base). Valores numéricos de entrada são encapsulados em String (`23` → `"23"`).
