...
O monomorfizador de generics deve tratar cadeias fluentes de forma geral: qualquer metodo generico que retorne outra classe generica concreta deve enfileirar a classe retornada para que os metodos genericos dela tambem sejam materializados. Essa regra nao e exclusiva de `TTList`, e usos com type parameters ainda abertos (`T`, `TOut`, `TAcc`) nao podem gerar instancias concretas, inclusive quando aparecem em comentarios/strings, linhas opacas ou simbolos sinteticos do indexador.
...
O provider de Code Actions deve recuperar diagnosticos da colecao `vscode.languages.getDiagnostics(document.uri)` quando o VS Code chamar `provideCodeActions` com `context.diagnostics` vazio, filtrando pelo range solicitado. Isso garante Quick Fixes a partir do painel Problems e de chamadas de range. `shared-return-global-function` possui Quick Fix especifico para introduzir uma temporaria, atribuir nela a chamada global e retornar a partir dessa variavel em `Shared Function`.

Imports desconhecidos devem passar pela validacao de modulos e emitir `module-not-found`/`module-not-declared`. O warning `unused-import` so deve ser emitido para namespaces ou classes resolvidos pelo `WorkspaceSymbolIndexer` ou pela System Library, evitando tratar modulo ausente como import nao utilizado. Adicionalmente, o linter detecta e acusa erros de referências circulares entre namespaces (`circular-import`), impedindo que um namespace importe a si mesmo ou forme ciclos com outros namespaces importados, embora o acesso qualificado a membros desses namespaces sem import explícito continue sendo válido para evitar referências circulares.

`D7AstContext.getMemberAccessContext()` deve filtrar candidatos pelo range da expressao antes de escolher o membro pelo nome. Linhas densas como `a.Target & b.Target.ToString()` nao podem resolver Definition/Completion do primeiro `Target` usando o receiver da segunda cadeia.

`TypeResolver.findInnermostClassSymbol()` e a selecao canonica de classe ativa para `Me`/`MyBase`. Ela deve ignorar classes sinteticas de generics e, quando houver ranges sobrepostos no indexador, escolher a declaracao textual mais interna para preservar campos homonimos como `BaseEnum._value` e `TEnum._value`. Quando houver classes com o mesmo nome em arquivos diferentes, como uma copia em `data7_modules/` e outra em `src/`, a resolucao contextual deve usar a identidade do `SymbolInfo` ativo e o `fileUri`, nao apenas o nome simples da classe.

Toda `Class` deve declarar pelo menos um construtor `Sub New`; `Structure` continua isenta. O diagnostico `missing-mybase-new` cobre dois casos: ausencia total de construtor (`action: "create-constructor"`) e construtor sem chamada a `MyBase.New()` (`action: "insert-mybase-new"`). O Quick Fix correspondente deve criar `Sub New()` sem parametros com `MyBase.New()` no **topo** da classe (logo apos a linha `Class`) ou inserir a chamada no construtor existente. `missing-mybase-free` continua inserindo `Sub Free()` antes do `End Class`.
...
Antes de iniciar o Executor no F5, a extensão reexecuta o pipeline de diagnosticos do projeto ativo **sem descartar** o cache quente do editor (AST/semântico). A execução é cancelada se houver qualquer erro de parser/linter restante. Avisos e hints `unused-code` nao bloqueiam. Quando a execução for cancelada, os diagnosticos publicados devem permanecer navegaveis e o painel Problems deve ser aberto automaticamente. Chamadas qualificadas contra namespaces ou tipos conhecidos tambem fazem parte dessa validação: `Namespace.Metodo()` deve emitir `unknown-member` quando o metodo nao estiver declarado no container, como em `console.clear()` quando o namespace `console` nao possui `clear`. O empacotamento aborta se o transpiler emitir Basic nativo que o parser sem plugins nao aceita; nao relinta semanticamente a saida materializada.
...
3. Um grafo reverso de dependências (`WorkspaceDependencyGraph`) que mapeia namespace → arquivo declarador e namespace → importadores, permitindo hot-reload: ao editar o arquivo A, apenas A e os arquivos que importam seus namespaces são re-lintados.
4. Debounce por documento (250 ms) no `DiagnosticService` para `onDidChangeTextDocument`, com contador de geração que descarta execuções obsoletas; `onDidSave` dispara refresh imediato com propagação a dependentes do grafo de `Imports` (`Principal.bas` espalha para todos os `.bas`); `fixActiveFile` força também a reavaliação de arquivos que ainda têm Problems. O overlay `unused-code` (reachability de projeto) é debounceado em 1500 ms. O comando **Rodar Linter no Projeto** limpa Problems e invalida caches de check antes de republicar (sem reparse síncrono prévio de todo o projeto). O F5 (`lintWorkspaceForRun`) **não** faz esse flush.
5. A exclusão da pasta gerenciada `data7_modules/` do FileSystemWatcher do VS Code para evitar auto-invalidações concorrentes.
6. Um cache compartilhado de parse em `LanguageProcessor.getOrParse`, usado por diagnostics, hover, completion, definition, references, rename, semantic tokens e quick fixes. Providers nao devem reparsear texto por conta propria quando esse cache atende ao caso; escritas feitas por correcoes devem chamar `LanguageProcessor.invalidate(uri)` para forcar uma leitura consistente na proxima interacao.
7. `SemanticLintCache` reutiliza o resultado completo de `runAdvancedDiagnostics` quando o conteudo do arquivo e as revisoes dos namespaces importados nao mudaram; o indexador ignora `updateFileContentFromParsed` quando o hash do conteudo e identico e invalida caches de membros apenas para tipos declarados no arquivo alterado.
8. `DeclarationLintCache` armazena diagnosticos por corpo de metodo/propriedade (hash do corpo + fingerprint de dependencias cross-file via `buildLintDependencyFingerprint`). O `DiagnosticsASTWalker` reutiliza entradas em cache hit e pula rules e walk da subarvore; invalidacao acompanha `SemanticLintCache` e dependentes do grafo de imports.
9. **Cold-path:** `LintUnitIndex` (prepasse unico para imports/unused/arrays), `LocalScopeIndex` (escopos locais por linha sem re-walk do AST), `FileLineContextIndex` (metodo/classe/propriedade por linha), cache de tipo de identificador por linha+nome, e `hasVariableInScope` para checagens de presenca sem substituicao generica redundante.
10. Caches de membros no `TypeResolver`/`WorkspaceSymbolIndexer` (`findClassSymbolCache`, `ownMembersForClassCache`, `inheritedMembersForClassCache`, `allMembersForTypeCache`) e filtro de `supportedNodeKinds` nas rules reduzem trabalho repetido no cold path do walker. O passe de generics do linter reusa a `CompilationUnit` já parseada.
11. `LintPipelineProfiler` para medir estágios do pipeline (parse, index-update, module-refs, advanced-lint, cache semântico, TypeResolver, publish, dependent-propagation). Habilitar com `DATA7_LINT_PROFILE=1` na extensão ou rodar `npm run lint:benchmark -w @data7/core` apontando `DATA7_BENCHMARK_WORKSPACE` para um projeto real (85 arquivos em `Modules/demo`: cold ~34s sequencial / ~32s lotes async / ~13s worker threads (8 cores) / ~109ms warm).
12. `lint-type-resolution-cache` consolida caches de MemberAccess, retorno de invocação, callables não qualificados e índices de escopo por `CompilationUnit`, invalidados em `LanguageProcessor.invalidate`, updates do indexador e dependentes do grafo. `lint-workspace-runner` + `lint-worker-pool` permitem varredura cold multi-core via snapshot (`exportLintSnapshot`); `resolveLintBatchConcurrency` ajusta lotes na extensão (`DATA7_LINT_CONCURRENCY`). O comando **Rodar Linter no Projeto** e `lintWorkspaceOnStartup` usam worker threads para arquivos fechados quando `DATA7_LINT_USE_WORKERS` ≠ `0`.
13. **Orquestração live:** lint debounced aguarda `markWorkspaceIndexReady()`; save dispara lint imediato com propagação coalescida a dependentes (lotes paralelos, sem re-disparo por arquivo na varredura completa); `fixActiveEditor`/`autoFixOnSave` suprimem refresh duplicado; fechar aba preserva diagnósticos de workspace lint; FSW não reindexa `.bas` com buffer sujo.

O pacote VSIX deve ser montado a partir de `packages/data7-vscode` como artefato fechado: o prepublish compila `@data7/core`, empacota `src/extension.ts` em `out/extension.js` com `esbuild`, gera `out/lint-worker.js` (worker thread do lint paralelo de workspace) e copia para staging apenas `docs/`, `core_modules/`, `out/lint-worker.js`, `out/mcp/server.bundled.js`, README, CHANGELOG e LICENSE. O comando de pacote usa `vsce package --no-dependencies` para evitar que symlinks/junctions de workspaces (`node_modules/@data7/*`) façam o VSIX incluir o monorepo inteiro ou artefatos antigos.

`Forms.Grid` usa indices **0-based** em `Cells` / `Col` / `Row` / `ColWidth` / `HideColumn` (nao 1-based). `ColCount` conta so colunas nao ocultas (`AllColCount` inclui ocultas; `VisibleColCount` e a viewport). `PermiteMarcarExclusao` cria uma coluna oculta extra no fim e deve ser a ultima operacao de estrutura. Delegates nativos: `TCanEditCellEvent` e `(Sender, ARow, ACol, ByRef CanEdit)` (linha, depois coluna); `TDblClickCellEvent` e `(pSender, pRow, pCol)` (linha, depois coluna); `TCellValidateEvent` tem `ByRef AValue` e `ByRef AValid`. `Forms.GridConfigs` inclui as flags booleanas legadas usadas para configurar linhas, sizing/moving, selecao e cliques de cabecalho do Grid. `Collections.TStrings` expoe `Item(Integer) As String` como indexed-property default, entao `TStringList` e descendentes aceitam acesso direto `lista[i]` e a forma com parenteses `lista(i)` no linter. Campos de instancia cujo nome coincide com um tipo da System Library (ex.: `Dim grid As Grid`) tem prioridade sobre o acesso estatico ao tipo. Identificadores que nomeiam um namespace (ex.: `migracoes.migrar`) nao sao sombreado por campos privados homonimos em outros modulos e nao emitem `missing-import` do container do campo; MemberAccess a tipos aninhados preserva o container qualificado (`pesquisaPadrao.Pesquisa` vs `modeloPesquisa.Pesquisa`). `StringList`/`TStringList`/`TStrings` sao tratados como familia compativel em APIs como `IO.File.GetFiles` e `Net.TFTP.List`. Casts `Tipo(expr)` usam lookup de classe (nao qualquer simbolo homonimo). A keyword `Global` e permitida como nome de Property.
...
O resolvedor de tipos considera variaveis declaradas no escopo superior como globais de projeto, com prioridade para `Principal.bas`; isso cobre usos legados como `_usuario.CodEmpresa` e `_modeloOperacaoConciliacao.CodCadastroOperadora` em outros arquivos. A escolha de overloads usa aridade e compatibilidade dos tipos dos argumentos, evitando selecionar o primeiro overload por quantidade de parametros quando os tipos diferem. O cache de resolucao de chamadas nao qualificadas inclui a posicao da chamada no arquivo e os tipos dos argumentos quando disponiveis, para que multiplas invocacoes do mesmo nome na mesma linha (ex.: `EnumToInt(TFileMode.Open()), EnumToInt(TFileAccess.Read())`) nao compartilhem o overload errado. Tipos primitivos correlacionados (`Char`/`String`, `Integer`/`Long`, etc.) sao aceitos como compativeis pelo linter; enums distintos que herdam de `TEnum` nao sao intercambiaveis.

Chamadas sem receiver sao resolvidas por escopo antes de cair em simbolos globais: classe ativa, arquivo atual, **mesmo Namespace em outros arquivos do projeto** (sem exigir `Imports`), namespaces importados, System Library e, por ultimo, simbolos sem container. Isso evita falsos `unknown-symbol`/`sub-used-as-function` quando uma `Sub`/`Function` vive em outro `.bas` do mesmo Namespace parcial, e evita falsos `sub-used-as-function` quando uma `Function` importada tem o mesmo nome de uma `Sub` em outro modulo. Acessos por receiver (`obj.Membro`) devem resolver membros a partir do simbolo concreto do tipo de `obj` no arquivo/escopo ativo; hover, autocomplete e linter nao devem usar um simbolo global homonimo de outro tipo como fallback quando o membro nao existe no receiver. A indexacao de expressoes `Variant` e `String` e permitida pelo linter para refletir o comportamento aceito pelo compilador Data7. `Net`, `TFTP`, `ftBinary` e `ftASCII` pertencem a System Library e nao devem promover `module-not-found`. Se uma variavel e tipada com um tipo inexistente ou inacessivel, o acesso a seus membros tambem deve gerar `unknown-member`, alem do `unknown-type` na declaracao. A excecao explicita e a diretiva de comentario `' data7:external-type <Tipo>`: em escopo de linha, bloco (`scope=block`) ou arquivo (`scope=file`), ela aceita tipos nativos/externos que nao estao indexados e trata variaveis declaradas com esses tipos como `Variant` para membro, hover e definicao.

Em listas de argumentos multiline, linhas compostas apenas por comentario sao trivia e nao podem ser tratadas como expressao pelo parser, linter ou formatter. Parametros e variaveis tipados como delegates sao callables: `pRegra(...)` deve validar aridade/tipos contra o delegate e retornar o tipo declarado pelo delegate, inclusive quando a chamada aparece dentro de classes genericas e cadeias fluentes como `ConectorDeAPI<T>.Parse<TOut>(...).Construir()`. Declaracoes locais dentro de lambdas de nivel de arquivo nao alimentam analise de uso da rotina externa. Se uma chamada sem receiver existir apenas em namespace nao importado, o linter deve emitir `missing-import`; o fallback global nao pode aceitar implicitamente funcoes declaradas em outro modulo.
...
Os Quick Fixes de `return-unrecommended` usam a linha atual do documento: um `Return` fora de condicional vira apenas atribuicao ao alvo do metodo/propriedade, enquanto retornos em condicionais tambem recebem o `Exit` correspondente. Em `Return` multilinha com continuacao (`+_`), o fix substitui o span inteiro da expressao (nao so a primeira linha) e reaproveita o texto-fonte original — preservando `()`/`CHAR(13)` e evitando linhas orfas de continuacao. Dentro de `Catch`, Function/Property preservam `Return`; quando o retorno por atribuicao e usado ali, `return-assignment-in-catch` emite warning e oferece troca para `Return`. O Quick Fix de `missing-mybase-free` insere `MyBase.Free()` imediatamente antes de `End Sub`, preservando todas as liberacoes de recursos anteriores. O Quick Fix de `missing-then` insere a palavra-chave no fim da expressão, antes dos espaços de alinhamento e do comentário inline, e deduplica diagnósticos equivalentes durante correções em massa.

Quick Fixes especificos devem ser listados antes das supressoes. Supressoes geradas pela extensao usam comentario proprio na linha anterior (`' data7:disable-next-line <code>`) para manter o codigo original intacto e marcar a linha exata do diagnostico; a sintaxe antiga `disable-line` continua aceita pelo parser de supressao para compatibilidade. `unreachable-declaration` deve emitir um diagnostico por bloco estaticamente inalcançavel e oferecer correcao para comentar o bloco inteiro. Para `finally-block-unsupported`, blocos `Finally` vazios sao removidos diretamente; blocos com conteudo continuam usando o fluxo de encapsular `Catch` com `If Assigned`.

Os diagnosticos de linguagem do demo `teste_arrays` cobrem recuperacao local de member access incompleto (`incomplete-member-access`), fechamento ausente de bloco (`unterminated-block`), regras endurecidas da linguagem nativa (`typed-const-unsupported`, `invalid-shared-member`, `redundant-public-modifier`, `loose-value-statement`, `invalid-declaration`, `incomplete-property-body`) e supersets de heranca usados somente pela extensao (`abstract-instantiation`, `sealed-inheritance`, `mustoverride-not-implemented`, `invalid-class-modifier-combination`). Member access terminado em `.` sem continuacao explicita `_` deve gerar erro local e nao consumir comentario/proxima linha; `unknown-member` com membro vazio nao deve ser emitido. `Overrides` deve vir em declaracao completa (`Sub`, `Function` ou `Property`) e apontar para membro herdado `Overridable` ou `MustOverride`; caso contrario, o linter emite `invalid-declaration`. Quick fixes de `redundant-public-modifier` e `unused-code` sao unitarios e explicitos; `unused-declaration` nao entra em autofix em massa/save/build. Campos `Private Shared` permanecem aceitos para estado estatico interno, especialmente no padrao `TEnum`; `Shared` publico/default em campo e `Shared` em propriedade/delegate/classe/estrutura sao erro. Campos tipados como delegate entram na resolucao como callables: chamadas `OnExecute(...)` validam a assinatura do delegate, atribuicoes de lambda ao campo exigem parametros exatos e hover/completion/semantic tokens mostram a assinatura callable. Completion, hover, definition, semantic tokens, references e rename devem ignorar invocacoes iniciadas dentro de comentarios ou strings por meio do helper compartilhado `isPositionInCommentOrString`.
...
O sugar `array-list` materializa `Dim itens[] As T` e parâmetros `pItens[] As T` como `TTList_T` e expande operacoes funcionais sobre listas conhecidas (`map`, `filter`, `find`, `findIndex`, `some`, `every`, `reduce`, `forEach`) antes da serializacao. Indexacao em lista conhecida aceita tanto `lista[i]` quanto a forma VB `lista(i)` — ambas baixam para `GetItem`/`SetItem` (TTList nao expoe indexer padrao por parenteses); o mesmo vale para campos sugar no receptor (`obj.Lista(i)` → `obj.Lista.GetItem(i)`). Atribuicoes `lista = []` / `lista = [a, b]` a um destino TTList (Dim ou campo) materializam `New TTList_T()` e `Push` por elemento. O linter tipa `lista(i)` como o elemento `T`, nao como a lista. O transpile injeta `Imports mod_tlist` automaticamente quando o sugar materializa `TTList_*`. Lambdas usam somente sintaxe VB-like (`Function(...) expr`, `Function(...) ... End Function`, `Sub(...) ... End Sub`); a sintaxe legada `=>` nao e suportada. O parser aceita lambdas como argumentos de chamadas em linhas separadas, inclusive em invocacoes genericas como `Reduce<Double>(Function(...) ... End Function, 0.0)`, e aceita cadeias multiline entre chamadas somente com continuacao explicita `_` apos o ponto (`lista._`, `Filter(...)._`, `Map(...)._`). Em blocos `Function` lambda, `Return` e a forma de devolucao de valor e nao deve emitir `return-unrecommended`; retorno por atribuicao ao nome da rotina nao e suportado porque a lambda nao possui nome. O linter cruza a lambda com o delegate esperado e emite `lambda-signature-mismatch` quando kind, parametros ou retorno nao batem; ao atribuir uma lambda diretamente a um campo delegate, a quantidade de parametros deve ser exata. O corpo da lambda continua sendo validado normalmente, incluindo tipos de argumentos em chamadas como `handler.Execute(valor, indice)`, mas nao entra no fluxo de `dead-code` do metodo externo. Referencias de metodo passadas para parametros delegate tambem sao comparadas pela assinatura esperada, inclusive quando o tipo do parametro ja esta materializado como `TFindDel_Integer`, em vez de comparar o retorno do metodo diretamente com o tipo do delegate. O suporte geral a lambda e independente do `array-list`: lambdas que nao sao consumidas pelo loop inline sao materializadas como metodos gerados na classe atual ou na helper `__Data7LambdaHost`; o metodo gerado deve copiar a assinatura do delegate esperado, adicionando parametros omitidos na lambda fonte e substituindo genericos (`T`, `TOut`, `TAcc`) pelo tipo concreto do receiver/metodo materializado. Quando a lambda anota um parametro com tipo qualificado (`ns.Tipo`), essa grafia e preservada no helper gerado — mesmo que a assinatura do delegate monomorfizado traga so o nome simples — para o `.bas` materializado compilar sem `Imports` do namespace. Dentro de `Function` lambda materializada, cada `Return expr` deve ser convertido para `Dim __retN As <tipo> = expr`, atribuicao ao nome da funcao gerada e `Exit Function`; a restricao de usar `Return` continua valendo somente no fonte da lambda. Helpers de lambdas globais declaradas dentro de `Namespace` devem ser emitidos dentro do mesmo namespace para preservar acesso a variaveis de escopo de modulo; helpers globais fora de namespace tambem devem ser preservados quando o wrapper sintatico `Sub __syntheticMethod` for removido. Quando uma `Function` retorna `TTList_*`, `Return lista.map(...)` e `Return lista.filter(...)` sao baixados para uma variavel temporaria, loop `For` e `Return` dessa temporaria. `TTList.Map<TOut>` deve inferir `TOut` pelo retorno da lambda quando nao for explicito, enquanto `Reduce<TAcc>` tambem respeita argumentos genericos explicitos e aceita `extra As Variant` apos o valor inicial. Cadeias materializadas como `Filter(...).Map_*(...).Reduce_*` devem propagar o tipo da lista intermediaria para que a lambda do `Reduce` receba a assinatura completa do delegate, e `Every` deve negar comparacoes como `Not (<comparacao>)` para preservar a precedencia do compilador Data7. O monomorfizador deve materializar metodos genericos membros dentro do tipo concreto do receiver, por exemplo `TTList_Integer.Map_String` e `TTList_Integer.Reduce_Double`, inclusive quando a chamada esta em outro modulo e o template esta em `data7_modules`. Com `array-list` habilitado, o linter trata `TTList<T>` e `TTList_*` como tipos runtime do sugar e nao exige `Imports mod_tlist` no fonte, pois o transpiler injeta esse import na saida materializada.

O pipeline de build mantém snapshots persistentes em `.data7/build-cache/` e pula o empacotamento quando as entradas (`src/`, `data7_modules/`, `data7.json`, opções de transpile/logger) e o `.7Proj` de saída continuam idênticos. Em rebuilds incrementais, o `Builder` reutiliza resultados de transpile por arquivo quando o conteúdo e a assinatura pública global não mudaram. O basename de cada `.bas` (exceto `Principal`) deve ser único em todo o projeto — é a chave da tag no `.7Proj` e de `modulesMetadata`; colisões em pastas diferentes falham o build em vez de sobrescrever o código. Namespaces parciais usam basenames distintos com o mesmo `Namespace`. A execução via F5 usa uma saída dedicada em `.data7/run/*.run.7Proj` para que a variante com logger não sobrescreva o `.7Proj` standard aberto pelo Developer Studio. O `Builder` inicializa as propriedades `virtualFolders` e `modulesMetadata` como vazias caso estejam ausentes no `data7.json` lido do workspace, garantindo estabilidade e evitando erros de tipo (`TypeError`) na compilação de projetos que não possuem estas propriedades declaradas. O empacotamento de dependências (`data7_modules/`) é feito de maneira recursiva: o `Builder` varre todas as subpastas físicas à procura de arquivos `.bas`, mapeando suas localizações físicas para a estrutura de pastas virtuais do `.7Proj` (atualizando o `data7.json` de saída e o XML final), garantindo que arquivos aninhados sejam devidamente compilados e referenciados. Durante o mapeamento das dependências, caso exista uma pasta física `src` dentro de qualquer módulo em `data7_modules`, o nível `src` é removido e todas as subpastas e arquivos abaixo dele são promovidos um nível na estrutura virtual do projeto final. Unidades de dependência sem declaração explícita de `Namespace` (incluindo `Principal.bas` de sub-módulos) são filtradas e ignoradas durante o empacotamento, impedindo que conflitem com o escopo global do projeto principal. O mapa de pastas virtuais é case-insensitive: no Windows, src/Modules e uma entrada legada modules no data7.json colapsam na mesma pasta (o ome segue a caixa do disco), evitando árvores duplicadas no XML. Antes de serializar `<Modulos>`, o `Builder` ordena as unidades por dependência de namespace via AST (`module-dependency-order.ts`): considera `Imports`, tipos qualificados (`mod_foo.TBar`) e chamadas qualificadas (`mod_foo.Printe`), priorizando usos em declaração sobre `Imports` quando há ciclos, para que o compilador nativo encontre cada namespace já declarado acima no XML.

O contrato de otimizacao vive em `data7.json#build.optimization`: `minify.enabled`, `minify.stripComments`, `minify.collapseWhitespace` (default `false`), `prune.enabled`/`prune.report`/`prune.alwaysInclude`/`prune.remove.*`, `uglify.enabled` e `sourceMap`. O passe `prune` (estrategia `principal-closure`) roda apos a transpilacao e remove declaracoes inalcancaveis a partir de `Principal.bas`/`Main` — namespaces, classes, structures, enums, delegates, methods, declare methods, fields, properties, consts, variables de namespace e `Imports` nao usados — cada unidade controlada por `prune.remove.*` (default `true`). Com `prune.enabled`, a descoberta de instanciações genericas no pre-transpile tambem se limita ao closure de namespaces vivos (evita monomorphs orfaos em `mod_tlist` vindos de dependencias podadas), mais os namespaces de runtime dos sugars (`mod_logger` via `logger-print`, que só aparece no fonte depois de `Print` → `mod_logger.Printe`). Com `prune.remove.localVariables` (default `true`), apos a poda por declaracao o build ainda remove `Dim`/`Const` locais sem uso e sem efeito colateral dentro de metodos/propriedades (ignora corpos com `OpaqueStatement`; nao remove binders de `Using`/`For`/`ForEach`/`Catch`). Seeds incluem `Main`, statements top-level de entrada em `Principal` (nao todo `Dim`/`Const`/`Function` so por existir), `@data7:keep*`/`entrypoint`/`external-api` e `alwaysInclude`. Globais do Principal so entram quando referenciados (`app.Run()` mantem `Dim app`; `GlobalDeadClass`/`deadApp`/`deadConst` caem). O Principal expoe apenas declaracoes top-level sem `ownerClass` aos outros modulos — membros de classes locais do Principal nao sao puxados por seeds nao qualificados (ex.: `.Free()`). Cadeias como `….Take().Value.AsString` propagam o membro pelo tipo de resultado do predecessor **ja vivo** (`Value` → `TTValue` → `AsString`), inclusive quando o namespace do tipo nao esta em `Imports` do caller — sem varrer todos os `Take`/`Data`/`Format`/`Item` do projeto (evita falso keep de `Ambient`/`mod_printer`/`HttpResponseData`/`ImageFormat`). Receivers locais (`Using _form As New TFormCard` / `Dim x As T` / parametros) resolvem `receiver.Member` pelo tipo declarado e pela cadeia `Inherits` (ex.: `_form.Show()` mantem `TFormBase.Show`). Chamadas `Namespace.Type.SharedMember` (ex.: `mod_card_stone.CardGroupersStone.GetOptions()`) resolvem o membro no tipo pelo nome quando o tipo ja foi semeado (ex.: via `Namespace.Type`), independentemente de `Imports` do namespace do tipo. Atribuicoes de evento (`Me.Handler`) e `Me`/`MyBase` preservam membros referenciados, inclusive helpers `Protected` herdados via cadeia `Inherits` em outro namespace. Membros `Overrides` em subclasses vivas sao mantidos quando o membro virtual correspondente na ancestralidade tambem esta vivo (despacho polimorfico). Quando um overload fica vivo, todos os siblings com a mesma chave sao analisados (assinaturas/corpos) para nao orphanar tipos (`FindDel`) nem helpers so referenciados em outra sobrecarga. `unusedImports` so remove `Imports` de namespaces do projeto mortos — imports de system library (`Collections`, `IO`, …) permanecem. Pipeline: prune → minify textual → uglify agressivo (nomes curtos). Com `sourceMap` (default `true`), o Builder grava `<projeto>.7Proj.map.json` (segmentos linha gerada→original compostos pelos passes, mais `symbols` do uglify) ao lado do `.7Proj` e em `.data7/build/`; com uglify tambem emite `<projeto>.uglify-map.json`. No VS Code, providers de Hover/Definition no idioma `data7project` leem esse mapa: F12 e hover em linhas dentro de `<Codigo>` abrem o `.bas` original e demangleiam nomes uglificados. Membros que colidem com a System Library (ex.: `Touch`) sao renomeados quando o receiver e tipo de usuario; em tipos de sistema o nome nativo permanece. `@data7:keep-name` / `Main`/`New`/`Free` / `Declare Lib` preservados. `opcoes.minify` e `opcoes.stripComments` permanecem como compatibilidade legada. As opcoes de otimizacao fazem parte da chave do snapshot de build. Novos projetos e decompilacoes recebem o bloco completo via `createDefaultProjectMetadata()`.

As configuracoes globais da extensao (Executor, features, sugars, exclude, diagnosticSeverity) deixam de viver em `settings.json` e passam para `extension-settings.json` no armazenamento global, editadas pelo comando `data7.settings.open` (webview). Na primeira ativacao, valores legados de `data7.*` no Settings UI sao migrados automaticamente.
...
As correções automáticas de sintaxe/estilo reutilizam o mesmo contrato de `source.fixAll.data7` no save, no comando `data7.linter.fixAllWorkspace` e, quando `data7.features.build.autoFixBeforeBuild` está ativo, no pipeline de build/execução. O caminho pré-build mantém fingerprints por workspace e só reavalia arquivos `.bas` alterados desde a última passagem; ele não é habilitado por padrão. O comando `data7.linter.fixAllWorkspace` resolve dinamicamente a primeira correção rápida (Quick Fix) aplicável a cada diagnóstico do projeto (ignorando ações de supressão), gera bulk fixes a partir de correções unitárias determinísticas e sempre percorre o projeto completo. Após o batch, o serviço **limpa toda a coleção Problems**, invalida caches de parse/check, reindexa os arquivos a partir do disco (e regrava `.data7/analysis-cache.json`) e **reanalisa** o conjunto — assim entradas stale e erros em dependentes não permanecem quando o fonte já foi corrigido. Cada escrita do batch também atualiza o `WorkspaceSymbolIndexer` (o pipeline evita eventos de documento). Alterações externas em `.bas` (watcher) agendam refresh debounceado apenas para arquivos que já tinham Problems publicados, sem relintar a árvore inteira. `DiagnosticCollection` reutiliza a Uri canônica no set/delete para evitar órfãos por casing no Windows.

O linter live acompanha arquivos físicos locais com esquema `file` que estejam abertos no editor, **somente em workspaces de projeto Data7 completo** (pasta aberta com `data7.json` na raiz). Fora desse contexto — por exemplo o monorepo da extensão ou um `.bas` avulso — o suporte básico de idioma (highlight, providers) pode ativar ao abrir o arquivo, mas indexação do workspace, linter e análise de dependências não rodam. Na ativação de um projeto Data7 a extensão agenda a indexação **depois** de `activate()` retornar (sem `withProgress` na ativação — esse padrão pode deixar o indicador preso no Extension Host). O progresso aparece na status bar; um timeout de 60s libera o linter mesmo se o scan falhar. Só então sincroniza `data7_modules/` (para não competir com o scan recursivo) e reanalisa os `.bas` já abertos. Em alterações/saves atualiza o arquivo correspondente, e ao fechar ou excluir o arquivo remove seus diagnósticos live da aba Problemas. A análise do workspace inteiro (`data7.linter.run`) limpa Problems, invalida caches e reindexa antes de republicar; no startup, quando `data7.features.diagnostics.lintWorkspaceOnStartup` está habilitado, o mesmo caminho fresco é usado. Diagnósticos de arquivos fechados permanecem no painel Problems até nova análise, limpeza explícita ou alteração/remoção do arquivo. Ao concluir, o comando manual exibe uma notificação informativa com o resumo dos diagnósticos e botões para executar correções em massa ou reiniciar a análise.

O gerenciamento de dependências no Data7 agora é totalmente explícito, assemelhando-se ao modelo do NPM. A varredura automática (auto-discovery) baseada na tag `@Module` e a sincronização em background baseada nos namespaces utilizados (`DependencyScanner`) foram removidas. As dependências devem ser explicitamente listadas no arquivo `data7.json` sob o campo `dependencies`.
O novo **ModuleOrchestrator** / **PackageManager** é responsável por listar, instalar, atualizar, remover e publicar módulos. O fluxo deve seguir o modelo do NPM: comandos aceitam um ou mais módulos, atualizam `data7.json#dependencies` com a versão resolvida, e então sincronizam `data7_modules/` como materialização local do manifesto. A sincronização sempre espelha também `core_modules/` em `data7_modules/core_modules`, independentemente de `data7.json#dependencies`, porque esses módulos são runtime obrigatório dos sugars e recursos comuns da linguagem. Projetos publicáveis podem declarar `data7.json#module.name` como nome canônico do pacote; esse nome é usado para publicação e para impedir self-install no projeto ativo. O catálogo é separado entre repositório local (`~/.data7/local_modules`) e repositório online (GitHub), com preferência de instalação pelo local quando ambos existirem. A sidebar do Gerenciador de Módulos consome esse catálogo, mostra módulos disponíveis/instalados/atualizáveis em seções local e online, e oferece checkboxes e botões para instalar, atualizar ou remover em lote. A publicação online deve fazer pré-checagem pública antes de autenticar no GitHub: se o módulo já existe sem alteração real, bloqueia; se há alteração com versão menor ou igual à publicada, exige bump de versão. O construtor encarrega-se de adicionar a tag `@Module-Imported` aos arquivos em `data7_modules/` para diferenciar dependências externas do código do usuário residente em `src/` durante o processo de build e decompilação.
Na mesma coleta AST, acessos abreviados de `With` representados como `.Membro` nao produzem referencia de modulo: o alvo vazio gerado pelo parser e descartado antes de alimentar diagnostics. O `WithScopeIndex` mapeia cada linha ao stack de expressoes `With` ativas (incluindo blocos aninhados) para resolver tipo de `.Membro`, autocomplete apos `.`, hover/definition e o fallback do linter que trata `Print .Text` como `Print(.Text)` sem parenteses.
...
- Para qualificar como iterável, o tipo de `<expr>` deve expor `Count` **ou** `Length` como `Integer` + um acessor inteiro (precedência `Items` > `GetItem` > `Take` > `Item` > `Strings` > `Objects`). `TTList<T>` qualifica via `Length` + `GetItem`/`Take` (o `Item As TTObject` herdado de `TTComposerList` não prevalece); `Collections.StringList` via `Count` + `Strings`.
...
- **Pedidos abertos são ignorados na coleta inicial**: usos que ainda dependem de parâmetro genérico aberto (`TTList<T>`, `Map<K>`) não entram sozinhos na fila global. Isso evita classes inválidas como `TTList_T`. Quando o template que os contém é instanciado com argumentos concretos (`TTMatrix<TGridCol, TGridRow, Variant>`), o Builder fecha os usos internos (`TTList<TypeRow>` / `Dim result[] As TypeRow`) e pede a materialização cruzada (`TTList_TGridRow` em `mod_tlist`). Sem esse fechamento, o arquivo do template reescreve o nome plano mas o dono do `TTList` nunca emite a classe.
- Suporta generics aninhados (`TList<TList<Integer>>` → `TList_TList_Integer`).
- **Metaprogramação em templates genéricos**: o parser preserva diretivas `<# IF ... THEN #>`, `<# ELSE #>` e `<# END IF #>` como statements opacos, e o monomorfizador AST as avalia depois da substituição dos argumentos concretos. Expressões suportadas: `TypeSystem.InheritsFrom(T, "Base")`, `TypeSystem.IsKind(T, "Delegate"|"Class"|"Structure"|"Primitive"|"Enum"|"Unknown")`, e atalhos `TypeSystem.IsDelegate(T)` / `IsClass(T)` / `IsStructure(T)` / `IsPrimitive(T)` / `IsEnum(T)`, todos com `NOT` opcional. Exemplo:
...
- Expandido para a classe específica do Enum que herda de `TEnum` (do namespace `mod_tenum`). A base herda de `TTObject`, preservando identidade, cópia e descarte para uso seguro em `TTList`. A classe gerada expõe Initialize lazy (`If TEnum._IsCached("<Enum>", "<primeiroItem>") Then Exit Sub`), Shared Function por valor, três overloads de `Load` (por instância, `Integer` e `String` via `X(TEnum._GetCache(...))`) e `GetOptions` — sem construtor local (usa o da base) e sem campo `_Initialized`. Valores numéricos de entrada são encapsulados em String (`23` → `"23"`).
- O **indexador** materializa a mesma superfície na análise (sem expandir a AST do lint), com base `mod_tenum.TEnum` para não conflitar com o homônimo legado `mod_enum.TEnum`, de modo que `AsString`/`IsValue`/`Load`/`GetOptions` resolvam em código que ainda usa a sintaxe `Enun`.
...
- O linter ignora `missing-mybase-new` e `missing-mybase-free` para classes que herdam de `TEnum`.
...
- Em módulos publicados/importados, `Principal.bas` é só entrypoint de desenvolvimento do pacote isolado: não é publicado sem `Namespace`, e na importação para `data7_modules/` qualquer `Principal.bas` remanescente é comentado por completo.
...
### 2.4.1. Catálogo e Publicação de Módulos

- Publicação local/online só inclui `.bas` com `Namespace` declarado; `Principal.bas` de desenvolvimento do módulo (sem Namespace) não entra no pacote. Na sincronização para `data7_modules/`, arquivos sem Namespace são descartados e `Principal.bas` remanescente é comentado por completo (entrypoint só no módulo isolado).
- O `ModuleOrchestrator` é o gerenciador de módulos estilo npm: `install`, `update` e `remove` aceitam um ou mais módulos, atualizam `data7.json#dependencies` e sincronizam `data7_modules/`.
- O catálogo online é derivado de releases do repositório GitHub de módulos. Só são listados/instalados módulos com tag válida no formato `<modulo>-v<versao>`.
- `RepositoryQueryService` mantém cache do catálogo online e a sidebar revalida em intervalo longo para evitar rate limit da API do GitHub.
- Projetos publicáveis podem declarar `data7.json#module.name` como nome canônico; o mesmo nome bloqueia instalação do próprio módulo no projeto ativo.
- Publicação online compara o conteúdo local com a release publicada antes da autenticação GitHub. Republicação sem alteração é bloqueada, e alteração exige versão maior.
- O manifesto publicado recebe `module.publisher` com o login GitHub autenticado. O unpublish online só é permitido para esse publisher ou para o dono do repositório de módulos e abre um PR removendo `modules/<modulo>`.
...
- O módulo `src/system-library/symbol-helpers.ts` fornece os helpers `buildClassSymbols`, `buildNamespaceSymbols`, `defineEnum`, `buildGlobalFunctions`, `param()`, as constantes `SYSTEM_RANGE`/`SYSTEM_URI` e a `UNSUP_NOTE`. Arquivos novos do system-library devem usar esses helpers em vez de repetir o boilerplate de `range`, `fileUri`, `isShared`, `isPrivate`, etc. — tipos enumerados em particular devem usar `defineEnum({ name, containerName?, description, values: [[nome, descrição], ...] })` em vez de chamadas soltas de `buildEnumVal` ou helpers locais.
...
- `unused-import` — uma diretiva `Imports` conhecida pelo workspace ou pela System Library não é referenciada pelo restante do arquivo.
...
- `duplicate-declaration` — um identificador foi declarado mais de uma vez no mesmo escopo ou conflita com uma declaração em outro escopo público e acessível (membro da classe, tipo do namespace, namespaces importados ou globais do sistema). Globais de `Principal.bas` não conflitam consigo mesmos (comparação de arquivo via `isSameFileUri`, não pela string da URI). Em classes, `Shared` e instância compartilham a mesma tabela de nomes: campo/propriedade e método com o mesmo nome colidem; overloads de método são aceitos quando a sequência de tipos dos parâmetros difere (assinatura idêntica → _Error_). O tipo de retorno pode divergir entre overloads (ex.: `First() As T` e `First(n) As TTList<T>`). Emite _Error_.
...
- `missing-return-type` — uma `Function` ou `Property` omitiu o tipo de retorno (`As <Tipo>`). Emite _Error_.
...
- `unused-code` — declaração (classe, método, campo, etc.) sem uso a partir de `Principal`/`Main`, diretivas `@data7:keep*` ou `build.optimization.prune.alwaysInclude`. Usa o motor `analysis/declaration-reachability` (mesmo do prune); as flags `prune.remove.*` controlam quais kinds são reportados. Emite _Hint_ com `DiagnosticTag.Unnecessary` (fade no editor) e payload `UnusedCodePayload`. `Sub New` / `Sub Free` acompanham a classe viva. Suprimível via `' data7:disable-line unused-code`.
- `unreachable-declaration` — código que não pode executar porque o fluxo já retornou/`Exit`/`Throw`, ou porque um condicional constante é sempre falso. Emite _Warning_ (com fade Unnecessary quando aplicável) e payload `UnreachableDeclarationPayload` (`{ code, startLine, endLine }`). Distinto de `unused-code` (declarações nunca referenciadas no grafo do projeto).
...
- `src/analysis/symbol-indexer.ts`: Indexador de arquivos `.bas` em segundo plano para o Workspace e parser de símbolos (`SymbolParser`, `WorkspaceSymbolIndexer`). Expõe `getAllFileSymbols()` para providers de busca workspace-wide (Reference, Rename). Acumula o delta de cada arquivo em um `FileChangeSet` (`recordChange` / `takeChangeSet` / `restoreChangeSet`) que só é fechado quando a propagação o consome, e invalida caches em duas camadas: edição de corpo derruba apenas as entradas do arquivo editado; mudança de API ou de namespace limpa tudo. As instanciações planas de genéricos (`TList_Integer`) são reexpandidas sob demanda quando o template chega ao índice depois do uso.
...
- `src/analysis/type-resolver.ts`: Resolução compartilhada de tipo de variável, classe qualificada e membros herdados (`TypeResolver`). Único módulo onde providers e linter consultam o catálogo de símbolos. `ClassResolutionContext` (arquivo ativo, namespace e `Imports`) orienta `findClassSymbol` quando há homônimos workspace/`core_modules`. `findParentClassSymbol` resolve `Inherits` não qualificado usando namespace local e `Imports` do arquivo declarante antes de homônimos da System Library; `findClassSymbol` prefere classes workspace válidas quando o nome simples colide com tipos nativos (ex.: `Control` workspace vs `TEditLink.Control`). `findMember` cacheia apenas por identidade da classe resolvida, não por nome simples ambíguo. Arrays nativos `Dim campo(n) As T` suportam indexação com parênteses `campo(i)`, invocação encadeada de delegates `campo(i)(args)` e membro `.Length`. `areDelegateSignaturesCompatible` aceita delegates com mesma lista de parâmetros mesmo quando os nomes dos tipos delegate diferem (`FindDelegate` vs `FindDel`). Subclasses workspace são aceitas onde o parâmetro declara o tipo base (`TEnum` → `BaseItem`).
- `src/analysis/analysis-host.ts`: Fronteira de ambiente (`AnalysisHost`) — documentos abertos, pastas do workspace, `fs` (sync + async) e log. `installAnalysisHost` / `getAnalysisHost`; implementações `createVscodeAnalysisHost` (extension host), `createNodeAnalysisHost` (CLI/MCP/testes) e `createLspAnalysisHost` em `packages/data7-lsp`.
- `packages/data7-lsp/`: Language Server Protocol (`@data7/lsp`). Processo stdio que reinstala o `AnalysisHost`, indexa o workspace e publica diagnósticos. Ativado pela extensão quando `features.diagnostics.useLanguageServer` é `true` (default `false`). Com a flag ligada, o Language Client é o **único** publicador do painel Problems (`diagnosticCollectionName: data7-lsp`); o `DiagnosticService` local não grava na collection `data7` (evita entradas duplicadas ao abrir um arquivo após lint de workspace). Ver `docs/rfcs/LSP-001-language-server.md`.
- `src/analysis/analysis-program.ts`: Orquestrador incremental (`AnalysisProgram`) que mantém um `FileSnapshot` **imutável** por arquivo (conteúdo, versão, hash, AST, bind e último check). Recebe ou herda o `AnalysisHost` ativo. `update` é o único ponto de entrada de edição; `closeDocument` libera memória mantendo o arquivo indexado, `deleteFile` o remove do índice e `renameFile` faz a troca atômica. Resultados de check são publicados atomicamente (guarda de versão + hash), nunca escritos no snapshot em uso, e o consumidor é avisado por `onCheckCompleted`.
- `src/analysis/check-scheduler.ts`: Fila de prioridade (`active` > `visible` > `open` > `dependent` > `transitive` > `background`) com driver de idle. Processa fatias de ~15 ms, cede o event loop entre elas, é interrompida por digitação, tem capacidade máxima com despejo do nível mais baixo e reenfileira trabalho cancelado.
- `src/analysis/file-snapshot.ts`: Contrato do `FileSnapshot` e da `AnalysisPriority`.
- `src/analysis/member-cache.ts`: Cache de resolução de membros que sabe qual arquivo declarou cada entrada, permitindo invalidar só o arquivo editado quando a mudança não altera API.
- `src/analysis/workspace-dependency-graph.ts`: Grafo reverso namespace → arquivos. Suporta múltiplos declarantes do mesmo namespace e expõe `getTransitiveDependents` para propagação completa de lint.
- `src/analysis/declaration-reachability/parse-cache.ts`: Memoização dos parses da análise de alcançabilidade, que roda sobre o projeto inteiro em cada ciclo de `unused-code`.
...
- `src/providers/formatter.ts`: Embelezamento de código (`D7BasicFormattingProvider`, `CodeFormatter`). O formatter combina pilha de blocos com contexto lexical de expressão para preservar indentação de chamadas/cadeias multiline, lambdas VB-like em argumentos, arrays literais, continuation `_` e linhas terminadas em `.`, sem depender de diagnósticos para decidir a formatação. Antes de indentar, uma fase lexical normaliza lambdas de bloco coladas a chamadas (`Metodo(Sub(...)` / `Metodo( Sub(...)`) para a forma canonica multiline, evitando que um `End Sub`/`End Function` interno feche o metodo externo por engano.
...
- `src/system-library/symbol-helpers.ts`: DSL semântica centralizada que elimina o boilerplate dos arquivos `*.ts` que descrevem classes, namespaces e enums nativos. Expõe `buildClassSymbols` (classe/`structure` + `properties`/`methods`, com suporte a `indexed-property`, `overloads`, `variadicParameters`, `isUnsupported`), `buildNamespaceSymbols` (namespace + funções/constantes/aliases globais), `defineEnum` (tipo enumerado inteiro — `kind: "class"` + um `kind: "variable"` por valor — em uma única chamada, substituindo os estilos legados de `buildEnumVal()` solto, helpers locais tipo `ak()`/`bs()` e literais manuais), `buildGlobalFunctions` (funções globais sem namespace, ex. `CStr`/`Print`), `param()` (atalho para `ParameterInfo`) e as constantes `SYSTEM_RANGE`/`SYSTEM_URI`/`UNSUP_NOTE`. Nenhum arquivo de símbolos deve declarar `range` local ou reimplementar esses builders — sempre importar daqui.
...
- `src/platform/vscode-api.ts`: Adapter puro para a pequena superficie de VS Code consumida pelo core (`Uri`, `Range`, `Diagnostic`, `workspace`, `window`, `commands`). O core importa este adapter, nunca `vscode` diretamente; `packages/data7-vscode/src/extension.ts` instala a API real durante `activate`, enquanto CLI/MCP usam a implementacao standalone.
...
npm run watch          # watch incremental dos tres workspaces + rebundle de extension.js / lint-worker.js (e MCP se o core mudou); usado pelo F5 da Extension Host. O lint de workspace publica na collection local e abre Problems quando há erros (salvo com useLanguageServer).
...
A superficie atual: **10 familias de Resources** (`data7://language/<chapter>`, `data7://system-library/<ns>`, `data7://examples/<path>`, `data7://diagnostics/codes`, `data7://idioms`, `data7://real-project/<file>`, `data7://official/<qualifiedName>`, `data7://guide/<slug>`, `data7://meta/snapshot`), **12 Tools** (7 lookup - incl. `data7_list_controls` - + 3 executable + 1 mixed + 1 cross-file lint) e **5 Prompts** (`data7_module_skeleton`, `data7_TEnum_pattern`, `data7_array_list_collection`, `data7_typed_recordlist`, `data7_form_skeleton`). Os tools executaveis reusam `SugarTranspiler` e `DiagnosticsLinter` diretamente sobre o adapter puro `src/platform/vscode-api.ts`; nao ha mais `installVscodeShim` nem hook de `require("vscode")` no runtime MCP. Sob `--workspace=<path>`, o servidor seed a `WorkspaceSymbolIndexer.createDetached()` para que o linter veja simbolos cross-file. Os 167 exemplos oficiais do ERP (extraidos de `docs/Documentacao Data7/**/*.html` por `scripts/extract-official-articles.js` e materializados em `out/mcp/data/articles.json`) sao consultaveis via `data7_get_official_example` e enriquecem `data7_describe_symbol` com signature + descricao + exemplo canonico em uma unica chamada - substituindo a necessidade de injetar 60+ k tokens em `AGENTS.md`.

**Orientacao para agentes (colecoes e namespaces):** o resource `data7://idioms` abre com um preamble de hierarquia de preferencia — para codigo novo, `Dim items[] As T` + sugar `array-list` + generics (`mod_tlist`) antes de `StringList` ou subclasse manual `TTList` com CType/delegates — e com regras obrigatorias de Namespace/Imports: nome `[A-Za-z_][A-Za-z0-9_]*` (sem pontos na declaracao); nunca `Imports` do proprio namespace; imports circulares proibidos (`circular-import`) — reestruturar ou usar `nome_do_namespace.Membro` qualificado. O prompt `data7_array_list_collection` gera o padrao moderno; `data7_typed_recordlist` permanece como fallback legado. `mod_card_grouper` em `data7://real-project/` e referencia historica de telas/dominio, nao de colecoes novas.
