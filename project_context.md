...
**Orientacao para agentes (colecoes):** o resource `data7://idioms` abre com um preamble de hierarquia de preferencia — para codigo novo, `Dim items[] As T` + sugar `array-list` + generics (`mod_tlist`) antes de `StringList` ou subclasse manual `TTList` com CType/delegates. O prompt `data7_array_list_collection` gera o padrao moderno; `data7_typed_recordlist` permanece como fallback legado. `mod_card_grouper` em `data7://real-project/` e referencia historica de telas/domino, nao de colecoes novas.
