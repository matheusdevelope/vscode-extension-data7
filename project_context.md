...
O contrato de otimizacao vive em `data7.json#build.optimization`: `minify.enabled`, `minify.stripComments`, `prune.enabled`/`prune.report`/`prune.alwaysInclude`, `uglify.enabled` e `sourceMap`. O passe `prune` (estrategia `principal-closure`) roda apos a transpilacao de todos os modulos — inclusive `data7_modules`, `core_modules` e modulos virtuais de sugar — e remove namespaces fora da cadeia alcancavel a partir de `Principal.bas`/`Main`; modulos sem namespaces vivos sao excluidos do `.7Proj`. Diretivas `@data7:keep`, `@data7:keep-name`, `@data7:entrypoint` e `@data7:external-api` preservam namespaces. `opcoes.minify` e `opcoes.stripComments` permanecem como compatibilidade legada para minificacao textual. As opcoes de otimizacao fazem parte da chave do snapshot de build. Novos projetos e decompilacoes recebem o bloco completo via `createDefaultProjectMetadata()`.
