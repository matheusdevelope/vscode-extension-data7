...
try {
  ...
  if (filesOutsideModules.length > 0) {
    console.error(
      "ERRO: Modificações de arquivos fora do diretório 'modules/' não são permitidas neste PR.",
    );
    console.error(
  ...
  if (moduleFolders.size > 1) {
    console.error(
      `ERRO: Apenas um único módulo pode ser adicionado ou alterado por Pull Request. Módulos detectados: ${Array.from(moduleFolders).join(", ")}`,
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(manifestPath)) {
    console.error(
      `ERRO: O arquivo 'data7.json' não foi encontrado na pasta do módulo: ${moduleDir}`,
    );
    process.exit(1);
  }
  ...
  if (manifest.nome.toLowerCase() !== moduleName.toLowerCase()) {
    console.error(
      `ERRO: O nome do diretório físico ('${moduleName}') deve ser idêntico ao campo 'nome' no 'data7.json' ('${manifest.nome}').`,
    );
    process.exit(1);
  }

  console.log(
    `Validação bem-sucedida para o módulo: ${manifest.nome} v${manifest.version || "1.0.0.0"}`,
  );
  process.exit(0);
} catch (err) {
