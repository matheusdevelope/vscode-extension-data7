...
- `src/mcp/` is the external MCP server (runs outside the extension host as a stdio child process). It may depend on `system-library/`, `analysis/`, `diagnostics/`, `project/`, `utils/`, and may import **type-only** from `infra/`. It must NOT import from `providers/`, `services/`, `extension.ts`, `infra/configuration.ts` (which depends on `vscode.workspace.getConfiguration`), nor `vscode` directly. The only sanctioned way for `src/mcp/` to consume VS Code API surface is via `src/mcp/runtime/vscode-shim.ts` (a runtime-installed shim that overrides `Module.prototype.require` for `"vscode"`). The fence is enforced by the `data7/mcp-isolation` block in `eslint.config.mjs`. Justified in [`docs/rfcs/MCP-001-mcp-server.md`](../../docs/rfcs/MCP-001-mcp-server.md).
...
- `docs/example/` is documentation-grade fixture data. Production sources (`src/**/*.ts` outside `src/test/`) must NOT `import` from it. Tests consume examples via `loadExample(...)` (filesystem read inside `src/test/_helpers/fixtures.ts`), never as ES modules. Enforced by the `docs-example-isolation` block in `eslint.config.mjs`.
